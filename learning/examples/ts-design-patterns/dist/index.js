...
require("./type-challenges"); // 直接运行类型测试
console.log('🦞 TypeScript 设计模式实战\n');
async function main() {
    // ==================== 单例模式 ====================
    console.log('=== 1. 单例模式 ===');
    const db = singleton_1.Database.getInstance('localhost', 27017);
    ...
    types.forEach(type => {
        const notifier = factory_1.NotificationFactory.create(type);
        notifier.send('工厂模式发送测试');
    });
    console.log();
    // ==================== 观察者模式 ====================
    console.log('=== 3. 观察者模式 ===');
    const emitter = new observer_1.EventEmitter({ count: 0 });
    emitter.subscribe({
    ...
    const service = new repository_1.UserService(repo);
